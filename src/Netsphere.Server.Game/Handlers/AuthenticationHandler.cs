...
namespace Netsphere.Server.Game.Handlers
{
          ...
          IHandle<CharacterFirstCreateReqMessage>
    {
        ...
        public async Task<bool> OnHandle(MessageContext context, LoginRequestReqMessage message)
        {
            ...
            {
                //Sezon 10 için burdan yürü allowedVersions
            ...
            // Validate session
            var sessionId = await _cacheClient.GetAsync<string>($"session_{message.SessionId}");
            ...
            {
                var accountId = long.Parse(sessionId.Value);
            ...
            //Fixed a bug which forces client to re-create characters when rejoining the server 
            //if (string.IsNullOrEmpty(accountEntity.Username))
            //{
              //  logger.Information("Trying to connect to null acc");
               // session.Send(new LoginReguestAckMessage(GameLoginResult.SessionTimeout));
               // return true;
            //wwww}
            ...
            var result = (string.IsNullOrWhiteSpace(account.Nickname) || session.Player.CharacterManager.Count == 0)
        ...
        public async Task<bool> OnHandle(MessageContext context, CharacterFirstCreateReqMessage message)
        {
            ...
            //var NickCheck = await _databaseService.<PlayerAccountInfoDto>(statement => statement
           // .Where($"{nameof(PlayerAccountInfoDto.Nickname):C} = @{nameof(message.CharacterName)}")
            //.WithParameters(new { message.CharacterName }))).FirstOrDefault();
