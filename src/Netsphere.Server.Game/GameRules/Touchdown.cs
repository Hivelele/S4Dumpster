...
namespace Netsphere.Server.Game.GameRules
{
    public class Touchdown : GameRuleBase
    {
        ...
        protected internal override void OnScoreTouchdown(Player plr)
        {
            ...
            //Çok istiyorsan þimdilik console.WriteLine ama lazým.
            Console.WriteLine($"Player {plr.Account.Nickname} has scored a TD");//eðer consola bu çýkmazsa demek ki buraya girmiyoz amk, ok? 

            //new master determination after each TD
            var plrs = Room.Players.Values.Where(x => x.State != PlayerState.Lobby && x.State != PlayerState.Waiting).ToArray();
            if (plrs.Length > 0)
            {
                var newMaster = plrs[new Random().Next(0, plrs.Length)];
                Room.ChangeMaster(newMaster);
                Room.ChangeHost(newMaster);
                Console.WriteLine($"Changed host to {plr.Account.Nickname} ");
            }

            //foreach (var bigpp in Room.Players.Values)
              //  Console.WriteLine(bigpp.Account.Nickname + ": " + bigpp.State.ToString());
            //bak þimdi, iðrenç ama debug yok.. el mecbur
            //bigpp.state.Flaccid
            //bunlarý kimse görmeden sil sonra
